...
export async function compressBuffer(buffer: Buffer, width = 800, height = 800, quality = 70): Promise<{ dataUrl: string }> {
  try {
    const compressed = await sharp(buffer)
      .resize(width, height, { fit: 'inside', withoutEnlargement: true })
      .jpeg({ quality })
    ...
    console.warn('Buffer compression failed:', e.message);
