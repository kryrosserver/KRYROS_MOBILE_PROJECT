    ...
    <div className="flex flex-col min-h-screen bg-[#f8fafc]">
      ...
      <header className="bg-white border-b border-slate-200 h-16 sticky top-0 z-30 flex items-center justify-between px-4 md:px-8">
        <div className="flex items-center gap-2 md:gap-4">
          ...
          <div className="hidden sm:block h-5 md:h-6 w-[1px] bg-slate-200"></div>
          <p className="hidden md:block text-sm text-slate-500">Business Insights & Controls</p>
        </div>
        
        <div className="flex items-center gap-1 md:gap-3">
          <button 
            onClick={() => { setIsRefreshing(true); load().finally(() => setTimeout(() => setIsRefreshing(false), 300)); }}
            className={`p-2 rounded-xl hover:bg-slate-50 transition-all min-w-[44px] min-h-[44px] flex items-center justify-center ${isRefreshing ? "text-green-600" : "text-slate-400"}`}
          >
            <RefreshCw className={`h-5 w-5 ${isRefreshing ? "animate-spin" : ""}`} />
          </button>
          <button className="p-2 rounded-xl text-slate-400 hover:bg-slate-50 transition-all relative min-w-[44px] min-h-[44px] flex items-center justify-center">
            <Bell className="h-5 w-5" />
            <span className="absolute top-2 right-2 h-2 w-2 bg-red-500 rounded-full ring-2 ring-white"></span>
          </button>
          <button className="p-2 rounded-xl text-slate-400 hover:bg-slate-50 transition-all min-w-[44px] min-h-[44px] flex items-center justify-center">
      ...
      <div className="p-4 md:p-6 lg:p-8 max-w-[1600px] mx-auto w-full space-y-6 md:space-y-8">
        ...
        <section className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 md:gap-6">
          ...
          <div className="flex flex-wrap items-center gap-2 md:gap-3">
            {quickActions.map((action, i) => (
              <Link 
                key={i} 
                href={action.href} 
                className="flex items-center gap-2 px-4 md:px-5 py-2.5 min-h-[44px] bg-green-500 hover:bg-green-600 text-white rounded-xl font-semibold transition-all shadow-lg shadow-green-500/20 active:scale-95 group"
              >
        ...
        {/* Core Stats Overview - Stack on mobile */}
        <section className="grid grid-cols-1 xs:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
          {summaryCards.slice(0, 4).map((card, i) => (
            <Link 
              key={i} 
              href={card.href} 
              className="bg-white p-4 md:p-6 rounded-2xl border border-slate-200 hover:border-green-500/30 hover:shadow-2xl hover:shadow-green-500/5 transition-all group relative overflow-hidden"
            >
              ...
              <div className="space-y-2 md:space-y-3 relative">
                <div className="flex items-center justify-between">
                  <span className="text-[10px] md:text-xs font-bold text-slate-400 uppercase tracking-widest">{card.title}</span>
                  <div className="p-1 md:p-1.5 bg-slate-50 rounded-lg group-hover:bg-green-50 group-hover:text-green-600 transition-colors">
                    <ChevronRight className="h-4 w-4" />
                  </div>
                </div>
                <div>
                  <h4 className={`text-xl md:text-2xl font-black ${card.color}`}>{formatPrice(card.value)}</h4>
                  <p className="text-[10px] md:text-[11px] text-slate-500 mt-1 font-medium">{card.subtitle}</p>
                </div>
              </div>
            </Link>
          ))}
        ...
        <div className="grid grid-cols-1 xl:grid-cols-12 gap-6 md:gap-8">
          ...
          <div className="xl:col-span-8 space-y-6 md:space-y-8">
            ...
            <div className="bg-white rounded-2xl md:rounded-3xl border border-slate-200 p-4 md:p-6 lg:p-8 shadow-sm">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-4 md:mb-8 gap-2">
                <h3 className="text-base md:text-lg font-bold text-slate-900">Business Management</h3>
                <Link href="/admin/settings" className="text-sm font-semibold text-green-600 hover:text-green-700">Customize Grid</Link>
              </div>
              <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-4 gap-3 md:gap-6">
                {modules.map((m, i) => (
                  <Link 
                    key={i} 
                    href={m.href} 
                    className="flex flex-col items-center gap-2 md:gap-3 group p-3 md:p-4 rounded-xl md:rounded-2xl hover:bg-slate-50 transition-all border border-transparent hover:border-slate-100"
                  >
                    <div className="p-3 md:p-4 bg-slate-50 text-slate-500 group-hover:bg-green-500 group-hover:text-white rounded-xl md:rounded-2xl transition-all shadow-sm group-hover:shadow-green-500/30 group-hover:-translate-y-1">
                      <m.icon className="h-5 w-5 md:h-6 md:w-6" strokeWidth={2} />
                    </div>
                    <span className="text-[10px] md:text-xs font-bold text-slate-700 group-hover:text-slate-900 transition-colors text-center">{m.title}</span>
                  </Link>
                ))}
              </div>
            </div>

            {/* Additional Stats Section - Stack on mobile */}
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 md:gap-6">
              {summaryCards.slice(4).map((card, i) => (
                <Link 
                  key={i} 
                  href={card.href} 
                  className="bg-white p-4 md:p-6 rounded-2xl border border-slate-200 hover:shadow-lg transition-all flex items-center justify-between group"
                >
                  <div className="space-y-1">
                    <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">{card.title}</p>
                    <p className={`text-lg md:text-xl font-black ${card.color}`}>{formatPrice(card.value)}</p>
                  </div>
                  <div className="h-9 md:h-10 w-9 md:w-10 bg-slate-50 group-hover:bg-green-50 rounded-xl flex items-center justify-center transition-colors shrink-0">
          ...
          {/* Sidebar Area - Stack on mobile */}
          <aside className="xl:col-span-4 space-y-6 md:space-y-8">
            <div className="bg-white p-4 md:p-6 lg:p-8 rounded-2xl md:rounded-3xl border border-slate-200 shadow-sm space-y-6 md:space-y-8">
              <div>
                <h3 className="text-base md:text-lg font-bold text-slate-900">Order Progress</h3>
                  ...
                  <div key={stat.label} className="space-y-2 md:space-y-3">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-2">
                        <div className={`h-2 w-2 md:h-2.5 md:w-2.5 rounded-full ${stat.color}`}></div>
                        <span className="text-xs md:text-sm font-bold text-slate-700">{stat.label}</span>
                      </div>
                      <span className="text-sm md:text-sm font-black text-slate-900">{stat.count}</span>
                    </div>
              ...
              <div className="pt-4 md:pt-8 border-t border-slate-100">
                <Link 
                  href="/admin/orders" 
                  className="flex items-center justify-center gap-2 w-full py-3 md:py-4 min-h-[44px] bg-slate-900 hover:bg-slate-800 text-white rounded-xl md:rounded-2xl font-bold transition-all hover:shadow-xl active:scale-[0.98]"
                >
                  Manage Orders
            ...
            <div className="bg-gradient-to-br from-green-600 to-emerald-600 p-4 md:p-6 lg:p-8 rounded-2xl md:rounded-3xl text-white shadow-xl shadow-green-600/20">
              <h4 className="font-bold text-base md:text-lg mb-2">Need Assistance?</h4>
              <p className="text-green-100 text-xs md:text-sm mb-4 md:mb-6 leading-relaxed">Our support team is available 24/7 to help you with any platform issues.</p>
              <button className="w-full py-3 min-h-[44px] bg-white text-green-600 rounded-xl font-bold hover:bg-green-50 transition-colors">
                Contact Support
              </button>
            </div>
          </aside>
