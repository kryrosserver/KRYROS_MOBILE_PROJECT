...
export default function ProductsPage() {
  ...
  const [form, setForm] = useState({
    ...
    variants: [] as { type: string; value: string; price: string; stock: string; sku: string }[],
  });
  ...
  const handleEditClick = async (p: Product) => {
    // Show partial data immediately if possible
    setEditItem(p);
    setEditForm({
      id: p.id,
      name: p.name,
    ...
    // Fetch full data in background to fill in missing pieces (all images, relations)
    const fullProduct = await fetchFullProduct(p.id);
    if (fullProduct) {
      setEditItem(fullProduct);
      setEditForm(prev => ({
        ...prev,
        description: fullProduct.description || prev.description,
        shortDescription: fullProduct.shortDescription || prev.shortDescription,
        images: Array.isArray(fullProduct.images) ? fullProduct.images.map((img: any) => typeof img === 'string' ? img : img.url) : prev.images,
        specifications: typeof fullProduct.specifications === 'string' ? JSON.parse(fullProduct.specifications) : (Array.isArray(fullProduct.specifications) ? fullProduct.specifications : prev.specifications),
        upsellProductId: String(fullProduct.productRelations?.[0]?.relatedId || prev.upsellProductId || ""),
        variants: Array.isArray(fullProduct.variants) ? fullProduct.variants.map((v: any) => ({
          type: v.type,
          value: v.value,
          price: String(v.price || ""),
          stock: String(v.stock || ""),
  ...
  const load = useCallback(async (search = "", skipCount = 0) => {
    setLoading(true);
    setError(null);
    try {
          ...
          <div className="flex-1 overflow-y-auto p-4 md:p-6">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 md:gap-8">
              ...
              <div className="space-y-6">
                ...
                <div>
                  ...
                  <div className="bg-slate-50 rounded-xl p-4 border border-slate-200 space-y-3">
                    ...
                    {currentCategoryAttributes.length > 0 && (
                      <div className="mb-4">
                        <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">Suggested for {categories.find(c => c.slug === form.categorySlug)?.name || 'this category'}</p>
                        <div className="flex flex-wrap gap-2">
                          {currentCategoryAttributes.map(attr => (
                            <button
                              key={attr}
                              onClick={() => {
                                if (!form.specifications.find(s => s.key === attr)) {
                                  setForm({ ...form, specifications: [...form.specifications, { key: attr, value: "" }] });
                                }
                              }}
                              className="px-3 py-1 bg-white border border-slate-200 rounded-full text-[10px] font-bold text-slate-600 hover:border-green-500 hover:text-green-600 transition-all"
                            >
                              + {attr}
                            </button>
                          ))}
                    ...
                    {form.specifications.length === 0 ? (
                      <p className="text-xs text-slate-400 text-center py-4 italic">No specifications added.</p>
                    ) : (
                ...
                <div>
                  <div className="flex items-center justify-between mb-3">
                    <label className="block text-sm font-semibold text-slate-700">Product Variants</label>
                    <button
                      onClick={() => setForm({ ...form, variants: [...form.variants, { type: "OTHER", value: "", price: "", stock: "", sku: "" }] })}
                      className="text-xs bg-blue-500 text-white px-3 py-2 rounded-lg font-bold hover:bg-blue-600 transition-colors min-h-[44px]"
                    >
                      + Add Variant
                    </button>
                  </div>
                  <div className="bg-slate-50 rounded-xl p-4 border border-slate-200 space-y-3">
                    {form.variants.length === 0 ? (
                      <p className="text-xs text-slate-400 text-center py-4 italic">No variants added.</p>
                    ) : (
                      form.variants.map((variant, idx) => (
                        <div key={idx} className="space-y-2 p-3 bg-white rounded-lg border border-slate-200 animate-in fade-in slide-in-from-top-1">
                          <div className="flex gap-2">
                            <select
                              value={variant.type}
                              onChange={(e) => {
                            ...
                            <input
                              ...
                              className="admin-input flex-[2] text-sm min-h-[44px]"
                            />
                            <button
                          ...
                          <div className="grid grid-cols-3 gap-2">
                            <input
                              ...
                              className="admin-input text-xs min-h-[36px]"
                            />
                            <input
                              type="number"
                              placeholder="Stock"
                ...
                <div>
                  <div className="flex items-center justify-between mb-3">
                    <label className="block text-sm font-semibold text-slate-700">Product Variants</label>
                    <button
                      onClick={() => setEditForm({ ...editForm, variants: [...editForm.variants, { type: "OTHER", value: "", price: "", stock: "", sku: "" }] })}
                      className="text-xs bg-blue-500 text-white px-3 py-2 rounded-lg font-bold hover:bg-blue-600 transition-colors min-h-[44px]"
                    >
                      + Add Variant
                    </button>
              ...
              onClick={async () => {
                ...
                try {
                  ...
                  Object.keys(form).forEach(key => {
                    ...
                    if (key === 'variants') {
                      formData.append(key, JSON.stringify(form.variants));
                      return;
                    }
                ...
                <div>
                  ...
                  <div className="bg-slate-50 rounded-xl p-4 border border-slate-200 space-y-3">
                    ...
                    {currentEditCategoryAttributes.length > 0 && (
                      <div className="mb-4">
                        <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">Suggested for {categories.find(c => c.slug === editForm.categorySlug)?.name || 'this category'}</p>
                        <div className="flex flex-wrap gap-2">
                          {currentEditCategoryAttributes.map(attr => (
                            <button
                              key={attr}
                              onClick={() => {
                                if (!editForm.specifications.find(s => s.key === attr)) {
                                  setEditForm({ ...editForm, specifications: [...editForm.specifications, { key: attr, value: "" }] });
                                }
                              }}
                              className="px-3 py-1 bg-white border border-slate-200 rounded-full text-[10px] font-bold text-slate-600 hover:border-green-500 hover:text-green-600 transition-all"
                            >
            ...
            <button
              onClick={async () => {
                ...
                try {
                  ...
                  formData.append("isTrending", String(editForm.isTrending));
                  formData.append("isHot", String(editForm.isHot));
                  formData.append("isBestSeller", String(editForm.isBestSeller));
                  ...
                  if (editForm.variants.length > 0) formData.append("variants", JSON.stringify(editForm.variants));
