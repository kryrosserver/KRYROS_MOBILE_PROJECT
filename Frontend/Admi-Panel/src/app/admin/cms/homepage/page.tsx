...
const SECTION_TYPES: SectionType[] = [
  ...
  { id: "CategoriesGrid", label: "Categories", icon: Grid, description: "Grid of featured categories", color: "#6366F1", bg: "rgba(18,214,197,0.12)" },
  ...
  { id: "CategoryProducts", label: "Category Products", icon: Grid, description: "Display a specific category with its products", color: "#6366F1", bg: "rgba(18,214,197,0.12)" },
];
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 6, fontSize: 11 }}>
        {[
          ...
          <div key={row.label} style={{ display: "flex", alignItems: "center", gap: 6 }}>
            ...
            <span style={{ fontWeight: 700, color: "#111827", marginLeft: "auto", paddingLeft: 10 }}>{row.val} ({row.pct}%)</span>
          </div>
        ))}
      </div>
    </div>
  ...
  return (
    ...
    <div style={{ color: "#111827", display: "flex", flexDirection: "column", gap: 16, paddingBottom: 80 }}>
      ...
      <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 16 }}>
        <div>
          <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 6 }}><Link href="/admin/cms" style={{ display: "flex", alignItems: "center", gap: 6, background: "#FFFFFF", border: "1px solid var(--card-border)", borderRadius: 8, padding: "5px 12px", color: "#4B5563", fontSize: 12, textDecoration: "none" }}><ArrowLeft style={{ width: 13, height: 13 }} /> Back to CMS</Link></div>
          <h1 style={{ fontSize: 22, fontWeight: 800, color: "#111827", whiteSpace: "nowrap", margin: 0 }}>Home Page</h1>
        </div>
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          <button onClick={loadSections}
            style={{ display: "flex", alignItems: "center", gap: 8, background: "#FFFFFF", border: "1px solid var(--card-border)", borderRadius: 10, padding: "9px 16px", color: "#4B5563", fontSize: 13, cursor: "pointer", fontWeight: 700 }}>
            <Eye style={{ width: 15, height: 15 }} /> Preview Home Page
          </button>
          <button onClick={hasChanges ? saveLayout : loadSections} disabled={saving}
            style={{ display: "flex", alignItems: "center", gap: 8, background: "#6366F1", border: "none", borderRadius: 10, padding: "9px 16px", color: "#0B1320", fontSize: 13, cursor: saving ? "not-allowed" : "pointer", fontWeight: 800, opacity: saving ? 0.7 : 1 }}>
            <Save style={{ width: 15, height: 15 }} />
            {saving ? "Saving..." : "Save Changes"}
          </button>
          <button style={{ width: 38, height: 38, background: "#FFFFFF", border: "1px solid var(--card-border)", borderRadius: 10, color: "#4B5563", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}>
      ...
      <div style={{ display: "flex", gap: 16, alignItems: "start" }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ background: "#FFFFFF", border: "1px solid var(--card-border)", borderRadius: 14, overflow: "hidden" }}>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "16px 20px", borderBottom: "1px solid var(--card-border)" }}>
              <div>
                <h2 style={{ fontSize: 13, fontWeight: 800, color: "#111827", margin: 0 }}>Home Page Sections</h2>
                <p style={{ fontSize: 11, color: "#4B5563", marginTop: 3 }}>Manage and customize the content sections displayed on your homepage.</p>
              </div>
              <Link href="/admin/cms/homepage/add-section"
                style={{ display: "flex", alignItems: "center", gap: 7, background: "#6366F1", border: "none", borderRadius: 10, padding: "8px 14px", color: "#0B1320", fontWeight: 800, fontSize: 12, cursor: "pointer", textDecoration: "none" }}>
            ...
            <div>
              {loading ? (
                [...Array(6)].map((_, i) => (
                  <div key={i} style={{ display: "flex", alignItems: "center", gap: 14, padding: "14px 20px", borderBottom: "1px solid var(--card-border)" }}>
                    ...
                    <div style={{ flex: 1, display: "flex", flexDirection: "column", gap: 6 }}>
                      <div style={{ height: 11, width: 160, borderRadius: 6, background: "#F9FAFB" }} />
                      <div style={{ height: 8, width: 100, borderRadius: 6, background: "#F9FAFB" }} />
                    </div>
                  </div>
                ))
              ) : sections.length === 0 ? (
                <div style={{ padding: "60px 20px", textAlign: "center" }}>
                  <Layout style={{ width: 36, height: 36, color: "#4B5563", opacity: 0.2, margin: "0 auto 10px" }} />
                  <p style={{ fontSize: 13, fontWeight: 700, color: "#4B5563", marginBottom: 14 }}>No sections created</p>
                  <button onClick={seedSections}
                    style={{ display: "inline-flex", alignItems: "center", gap: 7, background: "#FFFFFF", border: "1px solid var(--card-border)", borderRadius: 10, padding: "8px 16px", color: "#4B5563", fontSize: 12, fontWeight: 700, cursor: "pointer" }}>
                ...
                sections.map((section) => {
                  ...
                  return (
                    <div key={section.id}
                      style={{ display: "flex", alignItems: "center", gap: 14, padding: "14px 20px", borderBottom: "1px solid var(--card-border)", color: "#111827", transition: "background 0.1s" }}
                      onMouseEnter={e => { e.currentTarget.style.background = "#F9FAFB"; }}
                      onMouseLeave={e => { e.currentTarget.style.background = "transparent"; }}>
                      <GripVertical style={{ width: 14, height: 14, flexShrink: 0, cursor: "grab", color: "#4B5563" }} />
                      ...
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <p style={{ fontSize: 13, fontWeight: 700, color: "#111827" }}>{section.title || info.label}</p>
                        <p style={{ fontSize: 11, color: "#4B5563" }}>{info.description}</p>
                      </div>
                      <div style={{ fontSize: 11, flexShrink: 0, textAlign: "right" }}>
                        <span style={{ color: section.isActive ? "#16C784" : "#4B5563" }}>{countLabel}</span>
                        <br />
                        <span style={{ color: section.isActive ? "#16C784" : "#F59E0B", fontWeight: 700 }}>
                          {section.isActive ? "● Active" : "● Inactive"}
                        </span>
                      </div>
                      <div onClick={() => toggleActive(section)} style={{ width: 40, height: 22, borderRadius: 12, background: section.isActive ? "#16C784" : "#F9FAFB", position: "relative", cursor: "pointer", flexShrink: 0, transition: "background 0.2s" }}>
                      ...
                      <button onClick={() => {}}
                        style={{ width: 32, height: 32, borderRadius: 8, display: "flex", alignItems: "center", justifyContent: "center", color: "#4B5563", background: "transparent", border: "none", cursor: "pointer", flexShrink: 0 }}
                        onMouseEnter={e => { e.currentTarget.style.background = "#F9FAFB"; e.currentTarget.style.color = "#6366F1"; }}
                        onMouseLeave={e => { e.currentTarget.style.background = "transparent"; e.currentTarget.style.color = "#4B5563"; }}>
                        <Edit style={{ width: 14, height: 14 }} />
                      </button>
                      <button style={{ width: 32, height: 32, borderRadius: 8, display: "flex", alignItems: "center", justifyContent: "center", color: "#4B5563", background: "transparent", border: "none", cursor: "pointer", flexShrink: 0 }}
            ...
            {sections.length > 0 && (
              <div style={{ padding: "14px 20px", borderTop: "1px solid var(--card-border)", display: "flex", flexDirection: "column", alignItems: "center", gap: 4 }}>
                <button onClick={seedSections}
                  style={{ display: "flex", alignItems: "center", gap: 7, fontSize: 12, fontWeight: 700, color: "#4B5563", background: "transparent", border: "none", cursor: "pointer" }}
                  onMouseEnter={e => { e.currentTarget.style.color = "#6366F1"; }}
                  onMouseLeave={e => { e.currentTarget.style.color = "#4B5563"; }}>
                  <Plus style={{ width: 13, height: 13 }} /> Add New Section
                </button>
                <p style={{ fontSize: 10, color: "#4B5563" }}>Drag and drop sections to reorder</p>
              </div>
            )}
          </div>
        </div>
        ...
        <div style={{ width: 280, display: "flex", flexDirection: "column", gap: 12, flexShrink: 0 }}>
          <div style={{ background: "#FFFFFF", border: "1px solid var(--card-border)", borderRadius: 14, padding: "20px" }}>
            <h3 style={{ fontSize: 12, fontWeight: 800, color: "#111827", textTransform: "uppercase", letterSpacing: "0.06em", marginBottom: 14 }}>Home Page Overview</h3>
            <DonutChart active={activeSections.length} inactive={inactiveSections.length} />
          </div>

          <div style={{ background: "#FFFFFF", border: "1px solid var(--card-border)", borderRadius: 14, padding: "20px" }}>
            <h3 style={{ fontSize: 12, fontWeight: 800, color: "#111827", textTransform: "uppercase", letterSpacing: "0.06em", marginBottom: 10 }}>Quick Actions</h3>
            <div style={{ display: "flex", flexDirection: "column", gap: 2 }}>
              {[
                ...
                <button key={item.label} onClick={item.action}
                  style={{ width: "100%", display: "flex", alignItems: "center", gap: 10, padding: "9px 10px", borderRadius: 10, fontSize: 12, fontWeight: 700, color: "#4B5563", background: "transparent", border: "none", cursor: "pointer", textAlign: "left" }}
                  onMouseEnter={e => { e.currentTarget.style.background = "#F9FAFB"; }}
                  onMouseLeave={e => { e.currentTarget.style.background = "transparent"; }}>
          ...
          <div style={{ background: "#FFFFFF", border: "1px solid var(--card-border)", borderRadius: 14, padding: "20px" }}>
            <h3 style={{ fontSize: 12, fontWeight: 800, color: "#111827", textTransform: "uppercase", letterSpacing: "0.06em", marginBottom: 10 }}>Last Published</h3>
            {lastPublished ? (
              <div>
                ...
                <p style={{ fontSize: 11, color: "#4B5563", marginTop: 4 }}>by Admin User</p>
              ...
              <p style={{ fontSize: 11, color: "#4B5563" }}>No published sections yet</p>
            )}
          </div>

          <div style={{ background: "#FFFFFF", border: "1px solid var(--card-border)", borderRadius: 14, padding: "20px" }}>
            <h3 style={{ fontSize: 12, fontWeight: 800, color: "#111827", textTransform: "uppercase", letterSpacing: "0.06em", marginBottom: 10 }}>Tips</h3>
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              {[
                ...
                <div key={i} style={{ display: "flex", alignItems: "flex-start", gap: 8 }}>
                  <Info style={{ width: 13, height: 13, flexShrink: 0, marginTop: 1, color: "#6366F1" }} />
                  <p style={{ fontSize: 11, lineHeight: 1.5, color: "#4B5563" }}>{tip}</p>
                </div>
              ))}
            </div>
          </div>
