...
import { Plus, Search, FileEdit, Eye, Printer, Trash2, Bell, Calendar, Sun, Moon, Menu, ChevronDown } from "lucide-react";
      ...
      <div ref={innerRef} style={{ background: "var(--bg-primary)", color: "var(--text-primary)", display: "flex", flexDirection: "column", minHeight: "100vh" }}>
        <header style={{ background: "var(--bg-secondary)", borderBottom: "1px solid var(--card-border)", height: 60, display: "flex", alignItems: "center", justifyContent: "space-between", padding: "0 24px", gap: 16 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
            ...
            <h1 style={{ fontSize: 17, fontWeight: 700, color: "var(--text-primary)", whiteSpace: "nowrap", margin: 0 }}>Estimates</h1>
          </div>
          <div style={{ flex: 1, maxWidth: 340, position: "relative" }}>
            ...
            <input placeholder="Search estimates..." style={{ width: "100%", background: "var(--card-bg)", border: "1px solid var(--card-border)", borderRadius: 10, padding: "8px 40px 8px 36px", color: "var(--text-primary)", fontSize: 13, outline: "none" }} />
        ...
        <div className="space-y-6 pb-20" style={{ color: "var(--text-primary)", padding: "20px" }}>
          <div className="flex items-center justify-between">
            <div>
              <h2 className="text-2xl font-bold" style={{ color: "var(--text-primary)" }}>Estimates</h2>
              <p className="text-sm mt-1" style={{ color: "var(--text-secondary)" }}>Manage and track your customer estimates</p>
            </div>
            <Link href="/admin/estimate/new" className="btn-primary flex items-center gap-2"><Plus className="h-4 w-4" /> New Estimate</Link>
          </div>
          <div className="admin-card !p-0 overflow-hidden">
            <div className="overflow-x-auto">
              <table className="admin-table">
                <thead>
                  <tr><th>Estimate #</th><th>Client</th><th>Date</th><th>Valid Until</th><th className="text-right">Amount</th><th className="text-right">Actions</th></tr>
                </thead>
                <tbody>
                  {estimates.length > 0 ? estimates.map((est) => (
                    <tr key={est.id} className="group">
                      <td className="font-mono font-semibold text-sm" style={{ color: "var(--text-primary)" }}>#{est.number}</td>
                      <td className="text-sm" style={{ color: "var(--text-secondary)" }}>{est.clientId || "Walk-in Customer"}</td>
                      <td className="text-sm" style={{ color: "var(--text-muted)" }}>{new Date(est.createdAt).toLocaleDateString()}</td>
                      <td className="text-sm" style={{ color: "var(--text-muted)" }}>{est.validUntil ? new Date(est.validUntil).toLocaleDateString() : "—"}</td>
                      <td className="text-right font-bold" style={{ color: "var(--text-primary)" }}>{formatPrice(est.total)}</td>
                      <td className="text-right">
                        <div className="flex items-center justify-end gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                          {[{ Icon: Eye, title: "View", danger: false }, { Icon: Printer, title: "Print", danger: false }, { Icon: Trash2, title: "Delete", danger: true }].map(({ Icon, title, danger }) => (
                            <button key={title} title={title} className="p-2 rounded-lg transition-colors" style={{ color: danger ? "#EF4444" : "var(--text-muted)" }}
                              onMouseEnter={e => { e.currentTarget.style.background = danger ? "rgba(239,68,68,0.1)" : "var(--hover-bg)"; }}
                    ...
                    <tr><td colSpan={6} className="px-6 py-14 text-center">
                      <div className="flex flex-col items-center gap-3" style={{ color: "var(--text-muted)" }}>
                        ...
                        <Link href="/admin/estimate/new" className="text-sm font-bold underline" style={{ color: "#12D6C5" }}>Create your first estimate</Link>
                      </div>
                    </td></tr>
                  )}
                </tbody>
