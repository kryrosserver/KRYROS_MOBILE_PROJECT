...
function DarkModal({ title, onClose, children }: { title: string; onClose: () => void; children: React.ReactNode }) {
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ background: "rgba(0,0,0,0.7)", backdropFilter: "blur(6px)" }}>
      <div className="w-full max-w-lg rounded-2xl overflow-hidden" style={{ background: "#FFFFFF", border: "1px solid var(--card-border)", boxShadow: "0 25px 50px rgba(0,0,0,0.5)" }}>
...
export default function CountriesPage() {
  ...
  const stats = [
    { label: "Total Countries",  value: data.length,                                color: "#6366F1",    bg: "rgba(18,214,197,0.12)",  icon: Globe },
        ...
        <div className="space-y-6 pb-20" style={{ color: "#111827" }}>
          ...
          <div className="flex flex-row items-center justify-between gap-4">
            <div className="flex items-center gap-3">
              <Link href="/admin/locations-shipping" style={{ display: "flex", alignItems: "center", gap: 6, background: "#FFFFFF", border: "1px solid var(--card-border)", borderRadius: 8, padding: "5px 12px", color: "#4B5563", fontSize: 12, textDecoration: "none" }}>
              ...
              <div>
                ...
                <p className="text-sm mt-0.5" style={{ color: "#4B5563" }}>Manage supported countries and currencies</p>
              </div>
            </div>
            <div className="flex items-center gap-3">
                    ...
                    <tr key={item.id}>
                      <td>
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <p className="font-semibold text-sm" style={{ color: "#111827" }}>{item.name}</p>
                            <p className="text-xs font-mono mt-0.5" style={{ color: "var(--text-muted)" }}>{item.code}</p>
                          </div>
                        </div>
                      </td>
                      <td>
                        <p className="font-semibold text-sm" style={{ color: "#111827" }}>{item.currencyCode}</p>
                        <p className="text-xs mt-0.5" style={{ color: "var(--text-muted)" }}>{item.currencySymbol} · {item.symbolPosition}</p>
                      </td>
                      <td>
                        <p className="font-bold text-sm" style={{ color: "#111827" }}>{item.exchangeRate}</p>
                      ...
                      <td>
                        <div className="flex flex-col gap-1">
                          <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-lg text-xs font-semibold w-fit"
                            style={item.status ? { background: "rgba(22,199,132,0.12)", color: "#16C784" } : { background: "#F9FAFB", color: "var(--text-muted)" }}>
              ...
              <div className="flex items-center gap-1">
                ...
                {Array.from({ length: Math.min(5, totalPages) }, (_, i) => {
                  ...
                  return <button key={n} onClick={() => setPage(n)} className="h-8 w-8 rounded-lg flex items-center justify-center text-xs font-semibold"
                    style={n === page ? { background: "#6366F1", color: "#fff" } : { color: "var(--text-muted)" }}>{n}</button>;
                })}
              ...
              {[
                ...
                <label key={key} className="flex items-center gap-2 cursor-pointer select-none">
                  <div onClick={() => setF({ [key]: !(form as any)[key] } as any)}
                    ...
                    style={{ background: (form as any)[key] ? "#6366F1" : "#F9FAFB", border: `2px solid ${(form as any)[key] ? "#6366F1" : "#E5E7EB"}` }}>
