...
export default function GlobalMethodsPage() {
  ...
  const stats = [
    { label: "Global Methods",  value: data.length,                         color: "#6366F1",    bg: "rgba(139,92,246,0.12)",  icon: Settings2 },
    { label: "Active",          value: data.filter(d => d.isActive).length, color: "#16C784", bg: "rgba(22,199,132,0.12)",  icon: Truck },
    { label: "Always Free",     value: freeCount,                           color: "#6366F1", bg: "rgba(18,214,197,0.12)",  icon: Truck },
  ];
          ...
          <div className="flex flex-row items-center justify-between gap-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm mt-0.5" style={{ color: "#4B5563" }}>Default shipping rules applied across all locations</p>
              </div>
            </div>
            <div className="flex items-center gap-3">
          ...
          <div className="admin-card !p-0 overflow-hidden">
            <div className="overflow-x-auto">
              <table className="admin-table">
                ...
                <tbody>
                  {loading ? (
                    [...Array(5)].map((_, i) => <tr key={i}><td colSpan={7}><div className="h-5 rounded animate-pulse my-1 mx-2" style={{ background: "#F9FAFB" }} /></td></tr>)
                  ) : paginated.length === 0 ? (
                    <tr><td colSpan={7} className="text-center py-14">
                      ...
                      <button onClick={() => openModal()} className="inline-flex items-center gap-1 text-xs font-semibold mt-2" style={{ color: "#6366F1" }}>
                    ...
                    <tr key={item.id}>
                      <td>
                        <span className="text-xs font-mono px-2 py-0.5 rounded-lg" style={{ background: "#F9FAFB", color: "var(--text-muted)" }}>{item.sortOrder ?? 0}</span>
                      </td>
                      <td>
                      ...
                      <td>
                        <p className="font-bold text-sm" style={{ color: Number(item.fee) === 0 ? "#16C784" : "#111827" }}>
                      ...
                      <td className="text-sm" style={{ color: "#4B5563" }}>{item.estimatedDays || " "}</td>
                      <td>
