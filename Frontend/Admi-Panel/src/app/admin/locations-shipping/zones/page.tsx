  ...
  const handleMethodSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const body = {
        ...
        sortOrder: parseInt(String(methodForm.sortOrder)) || 0
      };
      const res = await fetch(url, {
                      ...
                      {item.shippingMethods?.map((m: any) => (
                        <div key={m.id} className="flex items-center justify-between text-[11px] bg-slate-50 p-2 rounded group/method">
                          <span className="font-bold text-slate-700">
                            <span className="px-1.5 py-0.5 bg-slate-200 text-[9px] rounded mr-2 text-slate-600">#{m.sortOrder ?? 0}</span>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md p-6 space-y-4">
            ...
            <div className="space-y-4">
              ...
              <div><label className="text-xs font-bold text-slate-500 uppercase tracking-widest">Sort Order (Position)</label><input type="number" placeholder="0" className="admin-input mt-1" value={methodForm.sortOrder} onChange={e => setMethodForm({...methodForm, sortOrder: parseInt(e.target.value) || 0})} /></div>
            </div>
