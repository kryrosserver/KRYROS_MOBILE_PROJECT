...
import { Plus, Edit, Trash2, RefreshCw, Truck, ChevronLeft, X, ChevronRight, PlusCircle, Globe, Map as MapIcon, Building2,
  ArrowLeft
} from "lucide-react";
...
function DarkModal({ title, onClose, width = "max-w-lg", children }: { title: string; onClose: () => void; width?: string; children: React.ReactNode }) {
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ background: "rgba(0,0,0,0.7)", backdropFilter: "blur(6px)" }}>
      <div className={`w-full ${width} rounded-2xl overflow-hidden`} style={{ background: "#FFFFFF", border: "1px solid var(--card-border)", boxShadow: "0 25px 50px rgba(0,0,0,0.5)" }}>
...
export default function ShippingZonesPage() {
  ...
  const stats = [
    { label: "Shipping Zones",   value: data.length,                          color: "#6366F1",    bg: "rgba(239,68,68,0.12)",   icon: Truck },
    { label: "Active Zones",     value: data.filter(d => d.isActive).length,  color: "#16C784", bg: "rgba(22,199,132,0.12)",  icon: Truck },
    { label: "Total Methods",    value: totalMethods,                          color: "#6366F1", bg: "rgba(18,214,197,0.12)",  icon: Truck },
  ];
  ...
  function LocationTag({ zone }: { zone: any }) {
    ...
    return <span className="px-2 py-0.5 rounded-lg text-xs font-semibold" style={{ background: "#F9FAFB", color: "var(--text-muted)" }}>Global Default</span>;
  }
          ...
          <div className="flex flex-row items-center justify-between gap-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm mt-0.5" style={{ color: "#4B5563" }}>Create regional shipping rules and pricing</p>
              </div>
            </div>
            <div className="flex items-center gap-3">
          ...
          <div className="admin-card !p-0 overflow-hidden">
            <div className="overflow-x-auto">
              <table className="admin-table">
                ...
                <tbody>
                  {loading ? (
                    [...Array(4)].map((_, i) => <tr key={i}><td colSpan={6}><div className="h-5 rounded animate-pulse my-1 mx-2" style={{ background: "#F9FAFB" }} /></td></tr>)
                  ) : data.length === 0 ? (
                    <tr><td colSpan={6} className="text-center py-14">
                      ...
                      <button onClick={() => openZoneModal()} className="inline-flex items-center gap-1 text-xs font-semibold mt-2" style={{ color: "#6366F1" }}>
                    ...
                    <tr key={item.id}>
                        ...
                        <div className="flex flex-col gap-1">
                          {item.shippingMethods?.map((m: any) => (
                            <div key={m.id} className="flex items-center justify-between gap-2 px-2 py-1.5 rounded-lg group/method"
                              style={{ background: "#F9FAFB", border: "1px solid var(--card-border)" }}>
                              <div className="flex items-center gap-2">
                                <span className="text-xs font-mono px-1.5 py-0.5 rounded" style={{ background: "#F9FAFB", color: "var(--text-muted)" }}>#{m.sortOrder ?? 0}</span>
                                <span className="text-xs font-semibold" style={{ color: "#111827" }}>{m.name}</span>
                                <span className="text-xs font-bold" style={{ color: "#6366F1" }}>${Number(m.price).toFixed(2)}</span>
                              </div>
                      ...
                      <td>
                        <span className="text-xs font-mono px-2 py-0.5 rounded-lg" style={{ background: "#F9FAFB", color: "var(--text-muted)" }}>{item.priority ?? 0}</span>
                      </td>
                      <td>
          ...
          <form onSubmit={handleZoneSubmit} className="space-y-4">
            ...
            <label className="flex items-center gap-2 cursor-pointer select-none">
              <div onClick={() => setZoneForm(f => ({ ...f, isActive: !f.isActive }))} className="h-5 w-5 rounded flex items-center justify-center cursor-pointer"
                style={{ background: zoneForm.isActive ? "#6366F1" : "#F9FAFB", border: `2px solid ${zoneForm.isActive ? "#6366F1" : "#E5E7EB"}` }}>
