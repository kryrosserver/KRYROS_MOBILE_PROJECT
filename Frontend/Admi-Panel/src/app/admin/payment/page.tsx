...
import { Plus, Search, CreditCard, Eye, Printer, Trash2, Bell, Calendar, Sun, Moon, Menu, ChevronDown } from "lucide-react";
      ...
      <div ref={innerRef} style={{ background: "var(--bg-primary)", color: "var(--text-primary)", display: "flex", flexDirection: "column", minHeight: "100vh" }}>
        <header style={{ background: "var(--bg-secondary)", borderBottom: "1px solid var(--card-border)", height: 60, display: "flex", alignItems: "center", justifyContent: "space-between", padding: "0 24px", gap: 16 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
            ...
            <h1 style={{ fontSize: 17, fontWeight: 700, color: "var(--text-primary)", whiteSpace: "nowrap", margin: 0 }}>Payments</h1>
          </div>
          <div style={{ flex: 1, maxWidth: 340, position: "relative" }}>
            ...
            <input placeholder="Search payments..." style={{ width: "100%", background: "var(--card-bg)", border: "1px solid var(--card-border)", borderRadius: 10, padding: "8px 40px 8px 36px", color: "var(--text-primary)", fontSize: 13, outline: "none" }} />
        ...
        <div className="space-y-6 pb-20" style={{ color: "var(--text-primary)", padding: "20px" }}>
          <div className="flex items-center justify-between">
            ...
            <Link href="/admin/payment/new" className="btn-primary flex items-center gap-2"><Plus className="h-4 w-4" /> Record Payment</Link>
          </div>
          <div className="admin-card !p-0 overflow-hidden">
            <div className="overflow-x-auto">
              <table className="admin-table">
                <thead>
                  <tr><th>Reference #</th><th>Client</th><th>Date</th><th>Method</th><th className="text-right">Amount</th><th className="text-right">Actions</th></tr>
                </thead>
                <tbody>
                  {payments.length > 0 ? payments.map((p) => (
                    <tr key={p.id} className="group">
                      <td className="font-mono font-semibold text-sm" style={{ color: "var(--text-primary)" }}>{p.reference}</td>
                      <td className="text-sm" style={{ color: "var(--text-secondary)" }}>{p.clientId || "Walk-in Customer"}</td>
                      <td className="text-sm" style={{ color: "var(--text-muted)" }}>{new Date(p.createdAt).toLocaleDateString()}</td>
                      <td className="text-sm capitalize" style={{ color: "var(--text-secondary)" }}>{p.method || "Cash"}</td>
                      <td className="text-right font-semibold" style={{ color: "#16C784" }}>{formatPrice(p.amount)}</td>
                      <td className="text-right">
                        <div className="flex items-center justify-end gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                          {[{ icon: Eye, title: "View", danger: false }, { icon: Printer, title: "Print", danger: false }, { icon: Trash2, title: "Delete", danger: true }].map(({ icon: Icon, title, danger }) => (
                    ...
                    <tr><td colSpan={6} className="px-6 py-14 text-center" style={{ color: "var(--text-muted)" }}>
                      <div className="flex flex-col items-center gap-3">
                        <CreditCard className="h-10 w-10 opacity-20" />
                        <p className="font-semibold text-sm">No payments recorded yet</p>
                        <Link href="/admin/payment/new" className="text-sm font-semibold underline" style={{ color: "#12D6C5" }}>Record your first payment</Link>
                      </div>
                    </td></tr>
                  )}
                </tbody>
