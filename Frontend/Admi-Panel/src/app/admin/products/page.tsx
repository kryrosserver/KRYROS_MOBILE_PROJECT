      ...
      <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 12, marginBottom: 20, flexWrap: "wrap" }}>
        ...
        <div style={{ display: "flex", gap: 10, flexShrink: 0 }}>
          <button onClick={() => { setIsRefreshing(true); load().then(() => setIsRefreshing(false)); }}
            ...
            <RefreshCw style={{ width: 14, height: 14, ...(isRefreshing ? { animation: "spin 1s linear infinite" } : {}) }} /> Refresh
          </button>
          <Link href="/admin/products/new"
            style={{ display: "flex", alignItems: "center", gap: 8, background: "#6366F1", borderRadius: 10, padding: "9px 18px", color: "#fff", fontWeight: 700, fontSize: 13, textDecoration: "none" }}>
            ...
            <thead>
              <tr style={{ background: "#F9FAFB", borderBottom: "1px solid #E5E7EB" }}>
                <th style={{ padding: "11px 16px" }}>
                  <input type="checkbox"
                    onChange={e => { if (e.target.checked) setSelected(new Set(paginated.map(p => p.id))); else setSelected(new Set()); }}
              ...
              {loading ? (
                [...Array(5)].map((_, i) => (
                  <tr key={i} style={{ borderBottom: "1px solid #F3F4F6" }}>
                    {[...Array(7)].map((_, j) => (
                      <td key={j} style={{ padding: "14px 16px" }}><div style={{ height: 14, borderRadius: 6, background: "#F3F4F6", width: j === 1 ? 140 : 70 }} /></td>
                    ))}
                  </tr>
                ))
              ) : paginated.length === 0 ? (
                  ...
                  <tr key={p.id} style={{ borderBottom: "1px solid #F3F4F6", background: selected.has(p.id) ? "#EEF2FF" : idx % 2 === 0 ? "#fff" : "#FAFAFA" }}>
                    ...
                    <td style={{ padding: "12px 16px" }}>
                      <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                        {p.images?.[0]?.url
                          ? <img src={p.images[0].url} alt={p.name} style={{ width: 40, height: 40, borderRadius: 8, objectFit: "cover", border: "1px solid #E5E7EB", flexShrink: 0 }} />
                          : <div style={{ width: 40, height: 40, borderRadius: 8, background: "#EEF2FF", display: "flex", alignItems: "center", justifyContent: "center", fontWeight: 700, color: "#6366F1", fontSize: 14, flexShrink: 0 }}>{p.name?.charAt(0)}</div>
                        }
                        <div style={{ minWidth: 0 }}>
                          <div style={{ fontWeight: 600, color: "#111827", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis", maxWidth: 180 }}>{p.name}</div>
                          <div style={{ fontSize: 11, color: "#9CA3AF" }}>{p.sku || "—"}</div>
                        </div>
                      </div>
                    </td>
                    <td style={{ padding: "12px 16px", color: "#6B7280", fontSize: 12, whiteSpace: "nowrap" }}>{p.category?.name || "—"}</td>
                    <td style={{ padding: "12px 16px", fontWeight: 600, color: "#111827", whiteSpace: "nowrap" }}>
                      ${(p.price || 0).toFixed(2)}
                      {p.salePrice ? <span style={{ fontSize: 11, color: "#EF4444", marginLeft: 4 }}>${p.salePrice.toFixed(2)}</span> : null}
                    </td>
                    <td style={{ padding: "12px 16px" }}>
                      <span style={{ fontSize: 11, fontWeight: 700, padding: "3px 8px", borderRadius: 20, background: sb.bg, color: sb.color }}>{p.stock} — {sb.label}</span>
                    </td>
                    <td style={{ padding: "12px 16px" }}>
                      <span style={{ padding: "3px 10px", borderRadius: 20, fontSize: 11, fontWeight: 700, background: p.isActive ? "#D1FAE5" : "#FEE2E2", color: p.isActive ? "#065F46" : "#991B1B" }}>
          ...
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "12px 16px", borderTop: "1px solid #F3F4F6", flexWrap: "wrap", gap: 8 }}>
            ...
            <div style={{ display: "flex", gap: 4 }}>
              <button disabled={page === 1} onClick={() => setPage(p => p - 1)}
                style={{ padding: "5px 10px", borderRadius: 7, border: "1px solid #E5E7EB", background: "#fff", cursor: page === 1 ? "not-allowed" : "pointer", color: "#374151", fontSize: 12, opacity: page === 1 ? .4 : 1 }}>
                <ChevronLeft style={{ width: 13, height: 13 }} />
              </button>
              {pageNums.map((n, i) => (
                <button key={i} disabled={n === "..."} onClick={() => typeof n === "number" && setPage(n)}
                  style={{ padding: "5px 10px", borderRadius: 7, border: "1px solid #E5E7EB", background: n === page ? "#6366F1" : "#fff", color: n === page ? "#fff" : "#374151", fontWeight: n === page ? 700 : 400, fontSize: 12, cursor: n === "..." ? "default" : "pointer" }}>
                  {n}
                </button>
              ))}
              <button disabled={page === totalPages} onClick={() => setPage(p => p + 1)}
                style={{ padding: "5px 10px", borderRadius: 7, border: "1px solid #E5E7EB", background: "#fff", cursor: page === totalPages ? "not-allowed" : "pointer", color: "#374151", fontSize: 12, opacity: page === totalPages ? .4 : 1 }}>
